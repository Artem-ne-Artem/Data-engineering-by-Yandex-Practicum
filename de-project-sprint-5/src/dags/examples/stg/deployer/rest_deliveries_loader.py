...
class DeliveryOriginRepository:
    ...
    def load_deliveries(self, sort_field: str, sort_direction: str, offset: int = 0, limit: int = 1000000) -> None:
        offset = int(offset or self.DEFAULT_OFFSET)

        api_str = "https://d5d04q7d963eapoepsqr.apigw.yandexcloud.net/deliveries?sort_field=%s&sort_direction=%s&limit=%d&offset=%d" % (
...
class DeliveryDestRepository:
    def insert_delivery(self, conn: Connection, id: str, val: Any):
...
class REST_DeliveryLoader:
    _LOG_THRESHOLD = 2
    WF_KEY = "deliveries_origin_to_stg_workflow"
    LAST_LOADED_ID_KEY = "last_loaded_id"
    BATCH_LIMIT = 1000  # инкрементальная загрузка

    def __init__(self, collection_loader: DeliveryOriginRepository, pg_dest: PgConnect,
                 pg_saver: DeliveryDestRepository, logger: Logger) -> None:
    ...
    def load_delivery(self):
        with self.pg_dest.connection() as conn:
            ...
            load_queue = self.collection_loader.load_deliveries(last_loaded_id, self.BATCH_LIMIT)
            ...
            i = 0
            for d in load_queue:
                self.pg_saver.insert_delivery(conn, str(d["order_id"]), str(d))
                i += 1
