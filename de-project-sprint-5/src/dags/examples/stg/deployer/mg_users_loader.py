...
class UserOriginRepository:
    ...
    def list_users(self, load_threshold: datetime, limit) -> List[Dict]:
...
class UserDestRepository:
    ...
    def insert_user(self, conn: Connection, id: str, update_ts: datetime, val: Any):
...
class MG_UserLoader:
    ...
    def __init__(self, collection_loader: UserOriginRepository, pg_dest: PgConnect, pg_saver: UserDestRepository,
                 logger: Logger) -> None:
    ...
    def load_user(self) -> int:
        ...
        with self.pg_dest.connection() as conn:
            ...
            load_queue = self.collection_loader.list_users(last_loaded_ts, self._SESSION_LIMIT)
            ...
            i = 0
            for d in load_queue:
                self.pg_saver.insert_user(conn, str(d["_id"]), d["update_ts"], d)
