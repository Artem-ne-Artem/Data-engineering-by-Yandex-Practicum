...
def upload_from_s3_to_pg(ti, nickname, cohort):
    ...
    while i <= df_activity_log.shape[0]:
        ...
        if df_activity_log.drop(columns=['id'], axis=1).loc[i:i + step].shape[0] > 0:
            ual_val = str([tuple(x) for x in df_activity_log.drop(columns=['id'], axis=1).loc[i:i + step].to_numpy()])[
                      1:-1]
...
select distinct
customer_id, max(first_name), max(last_name), max(city_id)
...
def update_mart_f_tables(ti):
    ...
    psql_conn = BaseHook.get_connection('pg_connection')
    conn = psycopg2.connect(
        f"dbname='de' port='{pg_conn.port}' user='{pg_conn.login}' host='{pg_conn.host}' password='{pg_conn.password}'")
    cur = conn.cursor()
...
select 
   dc.date_id,
   uol.item_id, uol.customer_id, uol.payment_amount/uol.quantity as price, sum(uol.quantity), sum(uol.payment_amount)
