# Используйте инструмент Callbacks в Airflow и добавьте функции, которые будут записывать в таблицу результаты работы проверок № 2 и 3
...
def check_success_insert_user_order_log2 (context):
    insert_dq_checks_results = PostgresOperator(
        ...
        sql = """
        INSERT INTO dq_checks_results
        values ('user_activity_log', 'check_row_count_user_order_log', current_date, 0)
        """
    )
...
def check_failure_insert_user_order_log2 (context):
    insert_dq_checks_results = PostgresOperator(
        ...
        sql = """
        INSERT INTO dq_checks_results
        values ('user_activity_log', 'check_row_count_user_order_log', current_date, 1)
        """
    )
